...
def hareruya_permutations_with_draw_and_byes_and_seat_factor(control_sheet, match_logs):
    control_sheet["Random"] = np.random.random(len(control_sheet))
    if not match_logs:
        control_sheet["Points"] = 1000
    ...
    for perm in tqdm(round_permutations):
        ...
        for match in match_log_perm:
            ...
            for seat, player in enumerate(match_players):
                if player == match_result:
                    pass
                elif match_result == "bye":
                    bye_players.append(player)
                else: # draw or loss
                    player_bet = control_sheet_perm.loc[player, "Points"]*0.07*seat_factor[seat+1]
                    points_for_winner += player_bet
                    control_sheet_perm.loc[player, "Points"] -= player_bet
    ...
    # account for byes
    # print(control_sheet.head().to_markdown())
    control_sheet["points_before_bye"] = control_sheet["Points"].copy()
    control_sheet["avg_point_move"] = (control_sheet["points_before_bye"] - 1000) / (max_round_number - control_sheet["bye"]) # -1 because one of the rounds was the bye
    control_sheet["avg_point_move"] = control_sheet["avg_point_move"].clip(lower=0)
    control_sheet.loc[control_sheet.avg_point_move.isna(), "avg_point_move"] = 0
    # print(control_sheet[control_sheet.Points.isna()].to_markdown())
    # print("####")
    control_sheet["Points"] = control_sheet["points_before_bye"] + control_sheet["avg_point_move"]*control_sheet["bye"]
...
def get_heuristic_pairing(control_sheet, pod_config, oppo_dict, lock_players):
    """
    ...
    # pre seat lock players
    for player, lock_table_id in lock_players.items():
        if player in players_byes: continue
        matches_4[lock_table_id-1].append(player)

    for player in players_4:
        # skip pre-seated players
        if player in lock_players.keys(): continue
...
def get_pairing_scores(
    ...
    for pod in pods_4P:
        for tuple in list(itertools.combinations(pod, r = 2)):
            if tuple[0] in oppo_dict[tuple[1]]:
                # print(tuple)
                rematch_counter += dict(Counter(oppo_dict[tuple[1]]).items())[tuple[0]]**2
    
    pod_stds = []
    control_sheet_copy = control_sheet.copy()
    for pod in pods_4P:
        temp = control_sheet_copy[control_sheet_copy.index.isin(pod)]
        pod_stds.append(temp.seat_avg.std())

    score = rematch_counter + (1-np.mean(pod_stds)/100)
...
def add_avg_seat(control_sheet, match_logs):

    control_sheet["seat_sum"] = 0
    control_sheet["seat_std"] = 0
    control_sheet["seatings"] = 0
    control_sheet["seatings"] = control_sheet["seatings"].astype(object)

    player_seatings = {}
    for player in control_sheet.index.unique():
        player_seatings[player] = []

    for match in match_logs:
        if match["match_result"] == "bye": continue
        for i, player in enumerate(match["match_players"]):
            player_seatings[player].append(i+1)
            # control_sheet.loc[player, "seat_sum"] += i+1

    for player, seatings in player_seatings.items():
        if not seatings: # in case of r1 bye
            seatings = [2.5]
        control_sheet.at[player, "seatings"] = seatings
        control_sheet.loc[player, "seat_sum"] = np.sum(seatings)
        control_sheet.loc[player, "seat_std"] = np.std(seatings)
        control_sheet.loc[player, "seat_avg"] = np.mean(seatings)

    # control_sheet["avg_seat"] = control_sheet.seat_sum / control_sheet.n_games    
